...
public:
	...
	int lower_bound;  // FOCAL's lower bound ( = e_weight * min_f_val)
	...
	*/
	...
	bool findPath(vector<PathEntry> &path, int f_weight, const vector < list< tuple<int, int, bool> > >* constraints, const bool* res_table, size_t max_plan_len, int lowerbound);
	inline void releaseClosedListNodes(hashtable_t* allNodes_table);
	SingleSearch(int start_location, int goal_location, const bool* my_map, int map_size, const int* moves_offset, int num_col, const vector<vector<int> > &cons_paths, int curr_time, int max_time);
	~SingleSearch();
};
